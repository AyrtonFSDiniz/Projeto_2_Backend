...
router.get("/listname/:nome", async (req, res) => {
  await Estado.find({ Nome: req.params.nome })
    .then((estados) => {
      res.status(200).json(estados);
    })
    .catch((err) => {
...
router.post("/create", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: `Estado: ${estado.Nome}, cadastrado com sucesso!` });
  } catch (err) {
...
router.put("/update/:id", async (req, res) => {
  ...
  await Estado.updateOne({ _id: id }, req.body)
    .then(() => {
      res
        .status(200)
        .json({ message: `Estado: ${req.body.Nome}, alterado com sucesso!` });
    })
    .catch((err) => {
      console.error(err);
      res
        .status(400)
        .json({ message: "Estado não encontrado, digite a ID corretamente." });
    });
});
